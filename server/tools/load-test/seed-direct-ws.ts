/**
 * 私聊 WS 路径：HTTP 登录两人 + 创建会话；发送阶段走 WS，并采样接收端 chat 下行延时。
 *
 * 用法:
 *   pnpm loadtest:ws:direct -- alice bob [--count=500] [--password=123456]
 *     [--start=now-1h] [--yes]
 */
import "./_lib/env";
...
async function main() {
  ...
  if (positionals.length < 2) {
    console.error(
      "用法: seed-direct-ws <userA> <userB> [--count=500] [--password=123456] [--start=now-1h] [--yes]"
    );
    process.exit(1);
  }
  ...
  console.log("[seed-direct-ws] config", {
    baseUrl,
    wsBaseUrl,
  ...
  await confirmOrExit(
    options,
    `将通过 WS 端到端在【${usernameA} <-> ${usernameB}】私聊发送 ${count} 条文字消息`
  );
  ...
  console.log("[seed-direct-ws] conversation", conv);
...
main()
  .catch(err => {
    console.error("[seed-direct-ws] failed", err);
    process.exitCode = 1;
  })
  .finally(async () => {
