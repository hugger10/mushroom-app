// Mirrors apps/web/src/utils/errorMessage.ts so that backend English
// messages (e.g. "Refresh token is invalid") surface as friendly Chinese
// copy in the mobile error banner / toasts. Keep the two files in sync
...
export function getReadableErrorMessage(
  ...
  if (normalized.includes("session has been superseded")) {
    return i18n.t("errorMessage.sessionSuperseded");
  }
  if (normalized.includes("session has expired")) {
    return i18n.t("errorMessage.sessionExpired");
  }
