 ...
 *
 * 设计目标：
 * - 上传进度可能 100ms 一次，写 SQLite/Electron DB 会引发明显写放大；
 *   仅在 0/100/失败这三个关键节点把状态落库（由 useChatOutgoing 处理），
 *   中间过程的视觉进度通过该 store 同步给 MessageBubble。
 * - 单例：附件以 clientMessageId 为 key，互不影响。
 * - 订阅基于 useSyncExternalStore，React 18 并发安全。
 */
